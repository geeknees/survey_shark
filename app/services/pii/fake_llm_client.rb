module PII
  class FakeLLMClient
    ...
    def mask_pii(text)
      ...
      # Mask companies - match company name only, preserving context
      masked.gsub!(/株式会社[^\s。、！？]*/, "[会社名]")
      masked.gsub!(/有限会社[^\s。、！？]*/, "[会社名]")

      # Mask schools - match school name only, preserving context
      masked.gsub!(/[^\s。、！？]*大学/, "[学校名]")
      masked.gsub!(/[^\s。、！？]*高校/, "[学校名]")
      masked.gsub!(/[^\s。、！？]*中学校/, "[学校名]")
      masked.gsub!(/[^\s。、！？]*小学校/, "[学校名]")
